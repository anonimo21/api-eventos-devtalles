...
import com.gestion.eventos.demo.service.EventService;
...
@WebMvcTest(controllers = EventController.class, excludeAutoConfiguration = {
                ...
                UserDetailsServiceAutoConfiguration.class // Excluye la configuración de seguridad principal y
                                                          // UserDetailsService
},
                // Añadimos excludeFilters para evitar que Spring escanee y cree tus beans de
                // seguridad específicos
                excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = {
                                JwtAuthenticationFilter.class,
                                JwtGenerator.class,
                                JwtAuthEntryPoint.class // Si JwtAuthEntryPoint también es un @Component y causa
                                                        // problemas
                }))
class EventControllerTest {
        ...
        void setUp(@Autowired EventService eventServiceMock, @Autowired EventMapper eventMapperMock) {
                ...
                Speaker speaker1 = new Speaker(20L, "Juan Pérez", "juan.perez@example.com", "Experto en Spring Boot.",
                                new HashSet<>());
                Speaker speaker2 = new Speaker(21L, "María García", "maria.garcia@example.com",
                                "Arquitecta de software.", new HashSet<>());
                ...
                SpeakerResponseDto speakerResponse1 = new SpeakerResponseDto(20L, "Juan Pérez",
                                "juan.perez@example.com", "Experto en Spring Boot.");
                SpeakerResponseDto speakerResponse2 = new SpeakerResponseDto(21L, "María García",
                                "maria.garcia@example.com", "Arquitecta de software.");
        ...
        void shouldReturnEventById() throws Exception {
                ...
                mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/events/{id}", 1L)
                                ...
                                .andExpect(jsonPath("$.speakers[?(@.name == 'Juan Pérez')].email")
                                                .value("juan.perez@example.com"))
                                .andExpect(jsonPath("$.speakers[?(@.name == 'Juan Pérez')].bio")
                                                .value("Experto en Spring Boot."))

                                // --- Verificación completa de María García (sin asumir si es [0] o [1]) ---
                                .andExpect(jsonPath("$.speakers[?(@.name == 'María García')].name")
                                                .value("María García"))
                                .andExpect(jsonPath("$.speakers[?(@.name == 'María García')].email")
                                                .value("maria.garcia@example.com"))
                                .andExpect(jsonPath("$.speakers[?(@.name == 'María García')].bio")
                                                .value("Arquitecta de software."));
        ...
        void shouldReturnNotFoundWhenEventDoesNotExist() throws Exception {
                ...
                mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/events/{id}", 99L) // Un ID que sabemos que no
                                                                                       // existe
                                .accept(MediaType.APPLICATION_JSON))
        ...
        void shouldReturnAllEventsPagedAndFiltered() throws Exception {
                ...
                // Preparación
                SpeakerResponseDto speakerResponseA = new SpeakerResponseDto(20L, "Juan Pérez",
                                "juan.perez@example.com", "Experto en Spring Boot.");
                ...
                mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/events")
                                ...
                                .andExpect(jsonPath("$.content[0].speakers[?(@.name == 'Juan Pérez')].name")
                                                .value("Juan Pérez"))
                                .andExpect(jsonPath("$.content[0].speakers[?(@.name == 'Juan Pérez')].email")
                                                .value("juan.perez@example.com"))
                                .andExpect(jsonPath("$.content[0].speakers[?(@.name == 'Juan Pérez')].bio")
                                                .value("Experto en Spring Boot."))
                                ...
                                .andExpect(jsonPath("$.content[1].speakers[?(@.name == 'Juan Pérez')].name")
                                                .value("Juan Pérez"))
                                .andExpect(jsonPath("$.content[1].speakers[?(@.name == 'Juan Pérez')].email")
                                                .value("juan.perez@example.com"))
                                .andExpect(jsonPath("$.content[1].speakers[?(@.name == 'Juan Pérez')].bio")
                                                .value("Experto en Spring Boot."))
        ...
        void shouldCreateEventSuccessfully() throws Exception {
                ...
                // Para el Event, necesitamos objetos Category y Speaker reales (o mocks si no
                // tuvieras los comunes)
                Category categoryForSavedEvent = new Category(10L, "Tecnología", "Eventos tecnológicos");
                Speaker speaker1ForSavedEvent = new Speaker(20L, "Juan Pérez", "juan.perez@example.com",
                                "Experto en Spring Boot.", new HashSet<>());
                Speaker speaker2ForSavedEvent = new Speaker(21L, "María García", "maria.garcia@example.com",
                                "Arquitecta de software.", new HashSet<>());
                ...
                // Paso 2: eventMapper.toResponseDto() es llamado con el Event devuelto y
                // devuelve un EventResponseDto
                ...
                verify(eventMapper, times(1)).toResponseDto(savedEventEntity); // Se verifica que el mapper fue llamado
                                                                               // con el Event correcto
        ...
        void shouldUpdateEventSuccessfully() throws Exception {
                ...
                // --- Datos del Event que el servicio MOCK debería devolver después de la
                // actualización ---
                // Este Event simula el estado del evento después de que eventService.update()
                // lo haya procesado.
                ...
                // Necesitamos la Category y el Speaker reales (o mocks) que corresponden a los
                // nuevos IDs
                Category newCategory = new Category(11L, "Desarrollo de Software", "Eventos de desarrollo");
                Speaker newSpeaker = new Speaker(22L, "Carlos López", "carlos.lopez@example.com",
                                "Experto en Frontend.", new HashSet<>());
                ...
                updatedEventResponseDto.setSpeakers(Set.of(new SpeakerResponseDto(22L, "Carlos López",
                                "carlos.lopez@example.com", "Experto en Frontend.")));

                // 2. Configurar el comportamiento de los mocks
                // Paso 1: eventService.update() es llamado y devuelve el Event actualizado
                when(eventService.update(eq(eventIdToUpdate), any(EventRequestDto.class)))
                                .thenReturn(updatedEventEntity);
                // Paso 2: eventMapper.toResponseDto() es llamado con el Event actualizado y
                // devuelve el EventResponseDto
                ...
                mockMvc.perform(put("/api/v1/events/{id}", eventIdToUpdate) // Endpoint para actualizar con el ID
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(updateEventRequestDto)))// El DTO con los
                                                                                                 // nuevos datos
                ...
                // 4. Verificación de interacciones con mocks:
                // Aseguramos que el método update del servicio fue llamado exactamente una vez
                // con el ID y cualquier EventRequestDto.
                verify(eventService, times(1)).update(eq(eventIdToUpdate), any(EventRequestDto.class));
                verify(eventMapper, times(1)).toResponseDto(updatedEventEntity); // Aseguramos que el mapper fue llamado
                                                                                 // con el Event actualizado

                // Verificaciones defensivas (que no se llamen otros métodos)
                verify(eventService, never()).save(any(EventRequestDto.class)); // No debería llamar a save al
                                                                                // actualizar
                verify(eventService, never()).findAll(anyString(), any(Pageable.class));
                verify(eventService, never()).findById(anyLong()); // El findById se hace internamente en
                                                                   // eventService.update, pero no lo llamamos
                                                                   // directamente aquí del controller
        ...
        void shouldReturnNotFoundWhenDeletingNonExistentEvent() throws Exception {
                ...
                mockMvc.perform(delete("/api/v1/events/{id}", nonExistentEventId))
                                ...
                                .andExpect(jsonPath("$.message")
                                                .value("Evento no encontrado con ID: " + nonExistentEventId));
