...
import com.gestion.eventos.demo.domain.Event; // ¡Importa la entidad Event!
import com.gestion.eventos.demo.repository.CategoryRepository;
import com.gestion.eventos.demo.repository.EventRepository; // ¡Importa el EventRepository!
...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Category conferencia = categoryRepository.findByName("Conferencia")
                .orElseGet(() -> {
                    Category newCat = new Category(null, "Conferencia",
                            "Eventos de gran escala con múltiples oradores.");
        ...
        // --- También puedes usar existsByName para verificar antes de crear como lo
        // tenías.
        // --- Lo he cambiado a findByName().orElseGet() para obtener las instancias
        // guardadas
        ...
        Speaker john = speakerRepository.findByEmail("john.doe@example.com")
                .orElseGet(() -> {
                    Speaker newSpeaker = new Speaker(null, "John Doe", "john.doe@example.com",
                            "Experto en desarrollo de software.", new HashSet<>());
                    return speakerRepository.save(newSpeaker);
                });
        Speaker jane = speakerRepository.findByEmail("jane.smith@example.com")
                .orElseGet(() -> {
                    Speaker newSpeaker = new Speaker(null, "Jane Smith", "jane.smith@example.com",
                            "Especialista en marketing digital.", new HashSet<>());
                    return speakerRepository.save(newSpeaker);
                });
        // Asegúrate de que los repositorios de Category y Speaker tengan métodos
        // findByName y findByEmail respectivamente.
